...
} = require('./manual-test-postgres-plan-revision-command-store-cloud');

const validEnv = {
  RUN_004I_PLAN_REVISION_COMMAND_VERIFY: CONFIRMATION,
...
const config = assert004iCloudEnvironment(validEnv);
...
assert.throws(
  () => assert004iCloudEnvironment({
    ...validEnv,
    RUN_004I_PLAN_REVISION_COMMAND_VERIFY: '',
  }),
