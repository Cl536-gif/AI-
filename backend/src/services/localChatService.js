...
function buildPrompt(question, perKb) {
  ...
  if (sections.length === 0) {
    return `用户问题：${question}\n\n（本地知识库这次没有检索到直接对应的资料，不能假装有资料支撑——` +
      '回答里要用自然口语化的说法，顺带如实告诉用户这一点，语气可以类似' +
      '"这块我目前没查到专门的资料，先凭经验给你个方向"这种程度（这只是示范' +
      '语气，不是让你原样照抄这句话），说完这句依然要正常回答用户的问题、给出' +
      '具体建议，不能因为没有资料支撑就回避回答，更不能说"建议咨询专业营养师"' +
      '"请换个问法"这类生硬的客服话术。）';
  }
