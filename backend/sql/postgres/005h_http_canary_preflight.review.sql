...
BEGIN
  ...
  IF v_identity_count <> 0 THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = '005h固定测试身份已存在，禁止在未清理状态下开始HTTP灰度';
  END IF;

  PERFORM set_config('app.verify_005h_preflight', 'PASS', true);
END
$verify$;

SELECT
  current_setting('app.verify_005h_preflight') AS status,
