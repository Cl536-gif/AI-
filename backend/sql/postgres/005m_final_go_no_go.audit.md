...
`NO_GO`。005a–005l已经形成可追溯证据，005m副作用恢复专项也已完成真实双实例云端验收；当前仍缺少以下三项真实运行证据：

1. 回滚信号触发CloudBase发布控制面的真实演练；
2. 百炼账户、余额/状态和模型请求失败的发布前监控门禁；
3. 独立预生产环境至少60分钟、至少100请求且七类失败/等待计数均为0的观察窗口。
