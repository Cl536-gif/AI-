  ...
  const prompt = [
    {
      ...
      content:
        ...
        '还有一类同样真实出现过的错误，务必避开：用户的回答只是把某一项' +
        '的"类别名称/话题词"本身说了一遍，并没有给出具体、可执行的实质内容——' +
        '比如问"有没有忌口"，用户只回了"忌口"两个字；问预算，用户只回了' +
        '"预算"两个字；问身材目标，用户只回了"身材目标"四个字。这种情况下，' +
        '用户并没有真正回答问题，只是复述了话题本身，这一项依然必须填 null，' +
        '不能把这个类别词直接当成答案内容存进去（错误示范：{"restrictions":' +
        '"忌口"}、{"budget":"预算"}、{"goal":"身材目标"}——这些都是把字段' +
        '自己的名字当成了字段的值，等于什么有效信息都没记录，却让系统误以为' +
        '已经拿到了答案，之后不会再追问，是很严重的错误）。判断标准很简单：' +
        '如果抽出来的候选值，读起来就是在重复"你问的是什么类别的问题"而不是' +
        '"具体是什么"，这个候选值就是无效的，必须填 null，让后续流程继续' +
        '追问具体内容。',
    },
