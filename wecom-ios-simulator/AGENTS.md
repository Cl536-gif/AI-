...
- Product decision: the simulator uses the LangGraph column logic from `backend/public/compare-v2.html`, while the app-owned UI remains enterprise-WeChat-like. Start with an empty transcript; send the user's first message directly to `/api/chat-langgraph` with the persistent `dietSecretary.deviceId`; reuse the returned `threadId` for later turns. Do not send `introAlreadyShown`, `privacyOnboarding`, `testPersona`, or channel service-choice state. A new conversation archives the transcript and resets only `threadId`; a separate explicit action creates a new device identity.
