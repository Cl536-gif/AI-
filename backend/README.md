...
**部署依赖**：这条链路直接 `require` 引用 `local-kb-tool/src/query.js`（跨项目共用同一份检索逻辑，没有复制粘贴）。正式 CloudBase 构建使用仓库根目录作为构建上下文，由根目录 `Dockerfile` 只复制检索所需的五个运行时模块、`diet`/`body-composition` 两个预构建索引，以及 BGE 查询嵌入模型的四个必要文件（约 23 MB）；不会复制原始 `.docx`、评测报告或 `local-kb-tool/node_modules`。模型包存在时运行时强制关闭远程模型下载，因此首个请求不依赖 Hugging Face 网络。`vectra` 和 `@xenova/transformers` 作为后端运行依赖安装在 `/app/node_modules`，满足 Node 的父目录模块解析规则；镜像保留 `/app/backend/src` 的目录层级，以保证对 `local-kb-tool` 的相对路径引用正确。基础镜像使用 Debian slim（而非 Alpine），以提供 `onnxruntime-node` 所需的 glibc 运行时。
