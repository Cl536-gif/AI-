...
`GO`（生产切换证据齐备）。005a–005l已经形成可追溯证据，005m副作用恢复专项、005n CloudBase控制面回滚演练、005o百炼模型依赖监控与005p独立预生产观察均已完成真实云端验收；当前证据阻塞项为0。

本决定不等于已经切换生产。正式服务仍保持SQLite，任何PostgreSQL全量切换都必须由用户另行明确授权，并继续通过能力、容量、回滚信号与最终确认门禁。
...
- 双实例正式HTTP路径连续60分钟、100/100成功请求、全部失败/等待信号为0及精确清理由`docs/acceptance/005p-preproduction-observation/`覆盖。
...
- 完成独立预生产观察窗口：不少于60分钟和100请求；readiness、连接超时、事务、身份、副作用、HTTP 5xx和池等待最大值均为0；已完成；
