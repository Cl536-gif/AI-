-- REVIEW ONLY: 005p独立预生产观察窗口前置检查。
-- 只读检查固定测试身份不存在；不输出摘要、用户ID或业务内容。
...
BEGIN
  ...
  IF v_identity_count <> 0 THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = '005p固定测试身份已存在，禁止在未清理状态下开始观察';
  END IF;

  PERFORM set_config('app.verify_005p_preflight', 'PASS', true);
END
$verify$;

SELECT
  current_setting('app.verify_005p_preflight') AS status,
