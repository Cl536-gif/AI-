-- 004f 云端只读前置检查。只返回对象状态和计数，不返回用户数据。
BEGIN;
...
WITH users_table AS (
  ...
  SELECT
    to_regclass('app.energy_calculations') IS NULL
      AS energy_calculations_table_absent,
    to_regprocedure(
      'app.record_current_user_energy_calculation(jsonb,timestamp with time zone)'
    ) IS NULL AS energy_calculation_function_absent
), permissions AS (
  SELECT
...
SELECT
  CASE
    WHEN current_database() = 'diet_secretary'
      ...
      AND energy_calculations_table_absent
      AND energy_calculation_function_absent
